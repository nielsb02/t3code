...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    options?: { readonly withoutOrchestration?: boolean; readonly sideChat?: boolean },
  ) =>
    Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            return Option.some(
              yield* decodeBrowserAccessThreadShell({
                ...
                ...(options?.sideChat ? { parentThreadId: asThreadId("parent") } : {}),
  ...
  it.effect("grants side-chat reporting independently of browser and device access", () =>
    Effect.gen(function* () {
      const threadId = asThreadId("side-chat-browser-off");
      assert.deepEqual(yield* startSessionWith(false, threadId, undefined, { sideChat: true }), [
        { threadId, capabilities: ["pull-requests", "side-chat"] },
      ]);
      assert.deepEqual(
      ...
      assert.deepEqual(yield* startSessionWith(true, threadId, undefined, { sideChat: true }), [
        { threadId, capabilities: ["device", "preview", "pull-requests", "side-chat"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
