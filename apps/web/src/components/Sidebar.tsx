...
export default function Sidebar() {
  ...
  const handleThreadFileDrop = useCallback(
    async (threadRef: ScopedThreadRef, files: File[]) => {
      ...
      const hasLanded = () => {
        const parentId = readThreadShell(threadRef)?.parentThreadId;
        const parentRef = parentId ? scopeThreadRef(threadRef.environmentId, parentId) : null;
        const routeRef = parentRef && readThreadShell(parentRef) ? parentRef : threadRef;
        if (
          ...
          return false;
        if (routeRef === threadRef) return true;
        const panel = useRightPanelStore.getState().byThreadKey[scopedThreadKey(routeRef)];
        return (
          panel?.isOpen === true && panel.activeSurfaceId === `side-chat:${threadRef.threadId}`
        );
      };
      const landedBefore = hasLanded();
